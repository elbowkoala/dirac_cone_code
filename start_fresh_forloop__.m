...
draw_y_range = round(290/e_bin):1:round(550/e_bin);
trial_DEs = round(310/e_bin):1:round(450/e_bin);
...
%     best_DV = trial_DVs(best_DV_i);
    ...
    %%
    examine_multi_domain_img = mat2gray(norman(imgaussfilt(corr_cone,4),0,4));
    examine_multi_domain_cut = examine_multi_domain_img(best__DK-10:best__DK+10,:);
    examine_multi_domain_sum = mat2gray(sum(examine_multi_domain_cut,1));
    min_under_bcb = min(examine_multi_domain_sum(120:170));
    exmdi = examine_multi_domain_img;
    
    tsh1 = round(590/e_bin);
    tsh2 = round(620/e_bin);
    top_span_ = mat2gray(sum(exmdi(:,tsh1:tsh2),2));
    top_span1 = k_bin*find(top_span_>=0.5,1,'first');
    top_span2 = k_bin*find(top_span_>=0.5,1,'last');
    top_span = top_span2 - top_span1;
    
    mins_bel_bcb(i) = min_under_bcb;
...
%         draw_MC_coeffs_map(:,i) = [best_DV; best_DE; best_DK];
        ...
        subplot(233), %plot(trial_DVs_units, best_I_of_DVs,'k'), hold on;
        ...
        subplot(236), imagesc((1:size(corr_cone,1)),fliplr(1:size(corr_cone,2)),rot90(norman(imgaussfilt(corr_cone,5),2,5))), axis xy, hold on;
        plot(plotfitx, -best__DV*(plotfitx-best__DK)+best__DE, 'c:'), hold on;
        plot(plotfitx, +best__DV*(plotfitx-best__DK)+best__DE, 'c:'), hold on;
        plot((100/max(examine_multi_domain_sum))*examine_multi_domain_sum,[1:length(examine_multi_domain_sum)],'w'), hold on;
        plot([1:size(corr_cone,1)],size(corr_cone,2)-50*top_span_,'c'), hold on;  
        plot([top_span1,top_span2],mean([tsh1,tsh2])*[1,1],'m:'),
        title({['min under BCB=',num2str(min_under_bcb)];...
            ['top span=',num2str(top_span)]},'FontSize',7)
        ...
        suptit = suptitle({['i=',num2str(i)];...%['MC: DE=',num2str(best_DE),' DV=',num2str(best_DV*(e_bin/k_bin)*(pix2eV/pix2invA))];...
            ...
            ['DPI=',num2str(DP_intensity_map(i))]});
...
%         plot(plotfitx, -best__DV*(plotfitx-best__DK)+best__DE, 'c:'), hold on;
%         plot(plotfitx, +best__DV*(plotfitx-best__DK)+best__DE, 'c:'), hold on;
%         plot(examine_multi_domain_sum,[1:size(corr_cone,2)],'w')
%         
%         suptit = suptitle({['i=',num2str(i)];...%['MC: DE=',num2str(best_DE),' DV=',num2str(best_DV*(e_bin/k_bin)*(pix2eV/pix2invA))];...
%             ['NC: DE=',num2str(best__DE),' DV=',num2str(best__DV*(e_bin/k_bin)*(pix2eV/pix2invA)),' MaxVal=',num2str(fit_evals_map(i))]});
%         set(suptit,'FontSize',8,'FontWeight','normal')
