...
tic
for i = 1:961%[round(961*rand),round(961*rand),round(961*rand),round(961*rand),round(961*rand)]%1:1:num_scans
    if rem(i,100)==0
        disp(['i=',num2str(i)])
        toc
    end    
    ...
    if DP_intensity_map(i) < mean(DP_intensity_map(:)) - std(DP_intensity_map(:))
    ...
    for trial_DE_i = 1:size(trial_DEs,2)
        ...
        for trial_DK_i = 1:length(trial_DKs)
            ...
            for trial_DV_i = 1:length(trial_DVs)                   
                ...
                NCC_denom = sqrt(sum(sum((X_matrix-Xm_mean).^2))) * sqrt(sum(sum((corr_cone_crop-ccc_mean).^2)));
...
%     best_DV = trial_DVs(best_DV_i);
    ...
    dirac_V_map(i) = best__DV*(e_bin/k_bin)*(pix2eV/pix2invA);
